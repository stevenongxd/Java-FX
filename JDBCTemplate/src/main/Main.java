...
//			connect.rs = connect.execQuery(query);
			...
			String query = String.format("SELECT * FROM registrant WHERE name = ? OR address = ?");
			PreparedStatement ps = connect.prepareStatement(query);
			
			try {
				ps.setString(1, viewTextField.getText());
...
//			code here (delete)
			String query = String.format("DELETE FROM `registrant` WHERE ID = %s", tempID);
			connect.execUpdate(query);
			refreshTable();
		...
		updateBtn.setOnAction((event) -> {
			...
			String query = String.format("UPDATE registrant SET name = '%s', address = '%s', age = '%s', ipk = '%s' WHERE id = '%s'", name, address, age, ipk, tempID);
			connect.execUpdate(query);
			refreshTable();
